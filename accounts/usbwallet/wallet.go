...
type wallet struct {
	...
	deriveChain    indigo.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain indigo.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
