...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Indigo protocol")
	statsFlag   = flag.String("netstats", "", "Netstats network monitoring auth string")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Indigo and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // Indigo address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Indigo light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Indigo protocol stack
	client *goclient.Client    // Client connection to the Indigo chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Indigo light client protocol
	...
	// Assemble the netstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightIndigo
			ctx.Service(&serv)
...
// close terminates the Indigo connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the Indigo address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
...
// the username, avatar URL and Indigo address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Indigo addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Indigo address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Indigo address and profile URL.
...
// returning the username, avatar URL and Indigo address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Indigo address,
