...
// geth is the official command-line client for Indigo.
package main
...
import (
	...
	"github.com/fulcrumchain/indigo/cmd/utils"
	"github.com/fulcrumchain/indigo/common"
	"github.com/fulcrumchain/indigo/console"
	"github.com/fulcrumchain/indigo/eth"
	"github.com/fulcrumchain/indigo/goclient"
	"github.com/fulcrumchain/indigo/internal/debug"
...
const (
	clientIdentifier = "Indigo" // Client identifier to advertise over the network
)

var (
	// Git SHA1 commit hash of the release (set via linker flags)
	gitCommit = ""
	// Indigo address of the Indigo release oracle.
	relOracle = common.HexToAddress("0xfa7b9770ca4cb04296cac84f37736d4041251cdf")
	// The app that holds all commands and flags.
	app = utils.NewApp(gitCommit, "the Indigo command line interface")
...
func init() {
	// Initialize the CLI app and start Indigo
	app.Action = indigo
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2018 The Indigo Authors"
...
func indigo(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full Indigo node is running
		if ctx.GlobalBool(utils.LightModeFlag.Name) || ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		var indigo *eth.Indigo
		if err := stack.Service(&indigo); err != nil {
			utils.Fatalf("Indigo service not running: %v", err)
		}
		...
		if threads := ctx.GlobalInt(utils.MinerThreadsFlag.Name); threads > 0 {
			...
			if th, ok := indigo.Engine().(threaded); ok {
				th.SetThreads(threads)
			}
		}
		if err := indigo.StartMining(true); err != nil {
