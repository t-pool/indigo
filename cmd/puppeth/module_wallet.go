...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	...
	echo $'geth --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --netstats \'{{.Netstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*"' >> wallet.sh
...
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	stats := &walletInfos{
		datadir:  infos.volumes["/root/.indigo"],
		nodePort: nodePort,
		rpcPort:  rpcPort,
		webHost:  host,
		webPort:  webPort,
		netstats: infos.envvars["STATS"],
	}
	return stats, nil
}
