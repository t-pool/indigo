...
import (
	...
	"github.com/fulcrumchain/indigo/common/fdlimit"
	...
	"github.com/fulcrumchain/indigo/dashboard"
	...
	whisper "github.com/fulcrumchain/indigo/whisper/whisperv5"
...
var (
	...
	TestnetFlag = cli.BoolFlag{
		Name:  "testnet",
		Usage: "Indigo test network: pre-configured test network",
	}
...
// for Indigo and returns half of the allowance to assign to the database.
...
// RegisterEthService adds an Indigo client to the stack.
func RegisterEthService(ctx context.Context, stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the Indigo service: %v", err)
	}
}
...
// RegisterNetStatsService configures the Indigo Stats daemon and adds it to
// the given node.
func RegisterNetStatsService(stack *node.Node, cfg netstats.Config) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both eth and les services
		var ethServ *eth.Indigo
		ctx.Service(&ethServ)

		var lesServ *les.LightIndigo
		ctx.Service(&lesServ)

		return netstats.New(cfg, ethServ, lesServ), nil
	}); err != nil {
		Fatalf("Failed to register the Indigo Stats service: %v", err)
	}
}
