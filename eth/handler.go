...
import (
	...
	"github.com/fulcrumchain/indigo/eth/fetcher"
...
// NewProtocolManager returns a new ethereum sub protocol manager. The Indigo sub protocol manages peers capable
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Indigo peer", "peer", id)

	// Unregister the peer from the downloader and Indigo peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Indigo protocol")
	...
	log.Info("Indigo protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Indigo peer connected", "name", p.Name())

	// Execute the Indigo handshake
	var (
	...
	if err := p.Handshake(pm.networkId, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Indigo handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Indigo peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Indigo message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the Indigo sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Indigo network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
