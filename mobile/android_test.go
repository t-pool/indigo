...
import (
	...
	"github.com/fulcrumchain/indigo/internal/build"
)
...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Indigo.LightScryptN, Indigo.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.indigo", new NodeConfig());
			node.start();
...
func TestAndroid(t *testing.T) {
	...
	// Generate the mobile bindings for Indigo and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "io.indigo", "github.com/fulcrumchain/indigo/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "io", "indigo", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
