...
import (
	...
	"github.com/fulcrumchain/indigo/accounts/usbwallet"
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Indigo", keep that.
...
var isOldIndigoResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	if c.name() == "geth" && isOldIndigoResource[path] {
		oldpath := ""
		if c.Name == "geth" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "indigo-keystore")
		ephemeral = keydir
	}
