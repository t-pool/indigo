...
func (m *manifestTrie) addEntry(entry *manifestTrieEntry, quitC chan bool) {
	m.hash = nil // trie modified, hash needs to be re-calculated on demand
	...
	if (oldentry.ContentType == ManifestType) && (cpl == len(oldentry.Path)) {
		if m.loadSubTrie(oldentry, quitC) != nil {
			return
		}
	...
	m.entries[b] = newManifestTrieEntry(&ManifestEntry{
		Path:        commonPrefix,
...
func (m *manifestTrie) deleteEntry(path string, quitC chan bool) {
	m.hash = nil // trie modified, hash needs to be re-calculated on demand
	...
	if (entry.ContentType == ManifestType) && (len(path) >= epl) && (path[:epl] == entry.Path) {
		if m.loadSubTrie(entry, quitC) != nil {
			return
		}
		...
		if cnt < 2 {
			...
			m.entries[b] = lastentry
		}
	}
}

func (m *manifestTrie) recalcAndStore() error {
	if m.hash != nil {
		return nil
	}
	...
	wg := &sync.WaitGroup{}
	key, err2 := m.dpa.Store(sr, int64(len(manifest)), wg)
	wg.Wait()
	m.hash = key
	return err2
}

func (m *manifestTrie) loadSubTrie(entry *manifestTrieEntry, quitC chan bool) (err error) {
	if entry.subtrie == nil {
		hash := common.Hex2Bytes(entry.Hash)
		entry.subtrie, err = loadManifest(m.dpa, hash, quitC)
		entry.Hash = "" // might not match, should be recalculated
	}
	return
}

func (m *manifestTrie) listWithPrefixInt(prefix, rp string, quitC chan bool, cb func(entry *manifestTrieEntry, suffix string)) error {
	plen := len(prefix)
	var start, stop int
	if plen == 0 {
		start = 0
...
func (m *manifestTrie) listWithPrefix(prefix string, quitC chan bool, cb func(entry *manifestTrieEntry, suffix string)) (err error) {
	return m.listWithPrefixInt(prefix, "", quitC, cb)
}

func (m *manifestTrie) findPrefixOf(path string, quitC chan bool) (entry *manifestTrieEntry, pos int) {

	log.Trace(fmt.Sprintf("findPrefixOf(%s)", path))

	if len(path) == 0 {
		return m.entries[256], 0
	}
...
func (m *manifestTrie) getEntry(spath string) (entry *manifestTrieEntry, fullpath string) {
	path := RegularSlashes(spath)
	var pos int
	quitC := make(chan bool)
	entry, pos = m.findPrefixOf(path, quitC)
