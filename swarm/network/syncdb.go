...
*/
func (s *syncDb) bufferRead(deliver func(interface{}, chan bool) bool) {
	...
	data, err := s.db.Get(s.counterKey)
	var counter uint64
	if err == nil {
		counter = binary.BigEndian.Uint64(data)
		log.Trace(fmt.Sprintf("syncDb[%v/%v] - counter read from db at %v", s.key.Log(), s.priority, counter))
	} else {
		log.Trace(fmt.Sprintf("syncDb[%v/%v] - counter starts at %v", s.key.Log(), s.priority, counter))
	}
...
LOOP:
	for {
		...
		case req = <-buffer:
			...
			more = deliver(req, s.quit)
			if !more {
				log.Debug(fmt.Sprintf("syncDb[%v/%v] quit: switching to db. session tally (db/total): %v/%v", s.key.Log(), s.priority, s.dbTotal, s.total))
			...
			s.total++
			log.Trace(fmt.Sprintf("syncDb[%v/%v] deliver (db/total): %v/%v", s.key.Log(), s.priority, s.dbTotal, s.total))
			...
			if len(buffer) == cap(buffer) {
				log.Debug(fmt.Sprintf("syncDb[%v/%v] buffer full %v: switching to db. session tally (db/total): %v/%v", s.key.Log(), s.priority, cap(buffer), s.dbTotal, s.total))
				buffer = nil
				db = s.buffer
			}
			continue LOOP
		...
		case req, more = <-db:
			if !more {
				...
				batch.Put(s.counterKey, counterValue) // persist counter in batch
				s.writeSyncBatch(batch)               // save batch
				log.Trace(fmt.Sprintf("syncDb[%v/%v] quitting: save current batch to db", s.key.Log(), s.priority))
				break LOOP
			}
			s.dbTotal++
			s.total++
		...
		case dbSize = <-s.batch:
			...
			if inBatch == 0 && quit != nil {
				...
				log.Debug(fmt.Sprintf("syncDb[%v/%v] empty db: switching to buffer", s.key.Log(), s.priority))
				db = nil
				buffer = s.buffer
			...
			binary.BigEndian.PutUint64(counterValue, counter)
			batch.Put(s.counterKey, counterValue)
			log.Debug(fmt.Sprintf("syncDb[%v/%v] write batch %v/%v - %x - %x", s.key.Log(), s.priority, inBatch, counter, s.counterKey, counterValue))
			batch = s.writeSyncBatch(batch)
		...
		case <-quit:
			...
			buffer = nil
			quit = nil
			log.Trace(fmt.Sprintf("syncDb[%v/%v] quitting: save buffer to db", s.key.Log(), s.priority))
			close(db)
			continue LOOP
		}

		// only get here if we put req into db
		entry, err = s.newSyncDbEntry(req, counter)
		if err != nil {
			log.Warn(fmt.Sprintf("syncDb[%v/%v] saving request %v (#%v/%v) failed: %v", s.key.Log(), s.priority, req, inBatch, inDb, err))
			continue LOOP
		}
		batch.Put(entry.key, entry.val)
		log.Trace(fmt.Sprintf("syncDb[%v/%v] to batch %v '%v' (#%v/%v/%v)", s.key.Log(), s.priority, req, entry, inBatch, inDb, counter))
		// if just switched to db mode and not quitting, then launch dbRead
		// in a parallel go routine to send deliveries from db
		if inDb == 0 && quit != nil {
			log.Trace(fmt.Sprintf("syncDb[%v/%v] start dbRead", s.key.Log(), s.priority))
			go s.dbRead(true, counter, deliver)
		}
		inDb++
		inBatch++
		counter++
		// need to save the batch if it gets too large (== dbBatchSize)
		if inBatch%int(s.dbBatchSize) == 0 {
			batch = s.writeSyncBatch(batch)
		}
	}
	log.Info(fmt.Sprintf("syncDb[%v:%v]: saved %v keys (saved counter at %v)", s.key.Log(), s.priority, inBatch, counter))
	close(s.done)
}

// writes the batch to the db and returns a new batch object
func (s *syncDb) writeSyncBatch(batch *leveldb.Batch) *leveldb.Batch {
	err := s.db.Write(batch)
	if err != nil {
		log.Warn(fmt.Sprintf("syncDb[%v/%v] saving batch to db failed: %v", s.key.Log(), s.priority, err))
		return batch
	}
...
func (s syncDbEntry) String() string {
	return fmt.Sprintf("key: %x, value: %x", s.key, s.val)
}
...
*/
func (s *syncDb) dbRead(useBatches bool, counter uint64, fun func(interface{}, chan bool) bool) {
	key := make([]byte, 42)
	copy(key, s.start)
	...
	for {
		...
		log.Trace(fmt.Sprintf("syncDb[%v/%v]: new iterator: %x (batch %v, count %v)", s.key.Log(), s.priority, key, batches, cnt))
		...
		for n = 0; !useBatches || n < cnt; it.Next() {
			...
			// log.Trace(fmt.Sprintf("syncDb[%v/%v] - %v, batches: %v, total: %v, session total from db: %v/%v", s.key.Log(), s.priority, s.key.Log(), batches, total, s.dbTotal, s.total))
			more = fun(entry, s.quit)
			if !more {
				// quit received when waiting to deliver entry, the entry will not be deleted
				log.Trace(fmt.Sprintf("syncDb[%v/%v] batch %v quit after %v/%v items", s.key.Log(), s.priority, batches, n, cnt))
				break
			}
		...
		log.Debug(fmt.Sprintf("syncDb[%v/%v] - db session closed, batches: %v, total: %v, session total from db: %v/%v", s.key.Log(), s.priority, batches, total, s.dbTotal, s.total))
		s.db.Write(del) // this could be async called only when db is idle
		it.Release()
	}
}
...
func (s *syncDb) newSyncDbEntry(req interface{}, counter uint64) (entry *syncDbEntry, err error) {
	var key storage.Key
