...
func (a *DbAccess) get(key storage.Key) (*storage.Chunk, error) {
	return a.loc.Get(key)
}

// current storage counter of chunk db
func (a *DbAccess) counter() uint64 {
	return a.db.Counter()
}
...
func (a *DbAccess) iterator(s *syncState) keyIterator {
	it, err := a.db.NewSyncIterator(*(s.DbSyncState))
	if err != nil {
		return nil
	}
...
//have been evaluated
func (sp *SyncParams) Init(path string) {
	sp.RequestDbPath = filepath.Join(path, "requests")
}
...
func newSyncer(
	...
	for i := 0; i < priorities; i++ {
		s.keys[i] = make(chan interface{}, keyBufferSize)
		s.deliveries[i] = make(chan *storeRequestMsgData)
		// initialise a syncdb instance for each priority queue
		s.queues[i] = newSyncDb(db, remotekey, uint(i), syncBufferSize, dbBatchSize, s.deliver(uint(i)))
	}
...
func (s *syncRequest) String() string {
	return fmt.Sprintf("<Key: %v, Priority: %v>", s.Key.Log(), s.Priority)
}
